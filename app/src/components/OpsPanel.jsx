...
export default function OpsPanel({ berryScore, history, lotId }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/store/bc_dc_strictness', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(dcStrictness) }).catch(() => {})
  }, [dcStrictness])

  useEffect(() => {
    localStorage.setItem('bc_target_score', targetScore.toString())
    fetch('/api/store/bc_target_score', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(targetScore) }).catch(() => {})
  }, [targetScore])
  ...
  const updateStrictness = (level) => {
    ...
    try {
      ...
      fetch('/api/store/bc_dc_log', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(log) }).catch(() => {})
    } catch {}
  }
