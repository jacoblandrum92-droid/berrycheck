    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <div style={{
            width: 7, height: 7, borderRadius: '50%',
            background: graderConnected ? COLORS.green : COLORS.text3,
            boxShadow: graderConnected ? `0 0 6px ${COLORS.green}` : 'none',
          }} />
          <div style={{
            fontFamily: FONT, fontSize: 10, color: graderConnected ? COLORS.green : COLORS.text3,
            letterSpacing: '0.08em',
          }}>
            {graderConnected ? 'GRADER' : 'NO GRADER'}
          </div>
        </div>

        {/* Scale status + live weight */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <div style={{
            width: 7, height: 7, borderRadius: '50%',
            background: scaleConnected ? COLORS.green : COLORS.text3,
            boxShadow: scaleConnected ? `0 0 6px ${COLORS.green}` : 'none',
          }} />
          {scaleConnected && scaleWeight != null ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{
                fontFamily: FONT, fontSize: 13, fontWeight: 700,
                color: COLORS.green, letterSpacing: '0.04em',
              }}>
                {scaleWeight.toFixed(1)}g
              </div>
              {onScaleTare && (
                <button onClick={onScaleTare} style={{
                  fontFamily: FONT, fontSize: 8, fontWeight: 600,
                  color: COLORS.text3, background: COLORS.bg3,
        ...
        {onFontScale && (
          <div style={{ display: 'flex', gap: 2 }}>
            {['small', 'normal', 'large'].map(size => (
              <button key={size} onClick={() => onFontScale(size)} style={{
                fontFamily: FONT, fontSize: size === 'small' ? 8 : size === 'normal' ? 10 : 13,
                fontWeight: fontScale === size ? 700 : 400,
                color: fontScale === size ? COLORS.green : COLORS.text3,
                background: fontScale === size ? COLORS.greenDim : 'transparent',
                border: `1px solid ${fontScale === size ? COLORS.green : COLORS.border}`,
