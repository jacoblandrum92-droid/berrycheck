...
export default function PhoneCapture() {
  ...
  const isDocument = new URLSearchParams(window.location.search).get('for') === 'document'
  ...
  const handleFile = (e) => {
    ...
    reader.onload = async (ev) => {
      ...
      if (sent) {
        ...
        // HTTP fallback — post image to relay
        try {
          const res = await fetch('/api/phone-image', {
            method: 'POST',
        ...
        {!preview ? (
          <>
            ...
            <div style={{
              ...
              {isDocument ? 'Camera ready. Photograph the document.' : 'Camera ready. Snap the tray.'}
            </div>
            ...
            <div style={{
              ...
              {isDocument ? 'TAP TO PHOTOGRAPH' : 'TAP TO CAPTURE'}
            </div>
          </>
        ) : (
          <>
            ...
            <button onClick={retake} style={{
              ...
              {isDocument ? 'NEXT PAGE' : 'NEXT SAMPLE'}
            </button>
          </>
        )}
      </div>
