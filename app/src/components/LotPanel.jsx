...
function saveCustomVariety(name) {
  try {
    ...
    if (!custom.includes(name) && !DEFAULT_VARIETIES.includes(name)) {
      ...
      fetch('/api/store/bc_varieties', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(custom) }).catch(() => {})
    }
  } catch {}
}
...
function saveClamshells(list) {
  ...
  fetch('/api/store/bc_clamshells', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(list) }).catch(() => {})
}
